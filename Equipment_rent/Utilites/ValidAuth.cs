...
internal class AuthClient
{
    ...
    public static async void AuthClient_Send(string username, string password)
    {
        ...
        {
            "Неверный логин", "Неверный пароль", "Авторизация прошла успешно", "При авторизации что-то пошло не так"
        };
        var mode = 0;
        var numberOfIteration = 99;
        var hashFunc = new Crypt();
        var salt_srv = Encoding.UTF8.GetBytes(sal);
        ...
        var data_log = Encoding.UTF8.GetBytes(mode.ToString() + '\r' + username + '\r' + hashedPass + '\n');
    ...
    {
        var ErrorMessages = new List<string>
            { "Неверный старый пароль", "Пароль успешно изменен", "Кажется что-то пошло не так" };
        var mode = 1;
        var numberOfIteration = 99;
        var hashFunc = new Crypt();
        var salt_srv = Encoding.UTF8.GetBytes(sal);
        ...
        var hashedLastPass =
            hashFunc.HashDataWithRounds(Encoding.UTF8.GetBytes(lastpassword), salt_srv, numberOfIteration);
        ...
        var data_change = Encoding.UTF8.GetBytes(mode.ToString() + '\r' + username + '\r' + hashedNewPass + '\r' +
                                                 hashedLastPass + '\n');
    ...
    public static async void CreateNewUser(string username, string password)
    {
        ...
        var response = new List<byte>();
        var bytesRead = 10;
        var data_change = Encoding.UTF8.GetBytes(mode.ToString() + '\r' + username + '\r' + hashedPass + '\n');
        await stream.WriteAsync(data_change);
        var Status = 10;
        string UserId;
        while ((bytesRead = stream.ReadByte()) != '\n')
        ...
        UserId = Encoding.UTF8.GetString(response.ToArray());
        if (Status == 0) MessageBox.Show("Пользователь с таким логином уже существует");
        if (Status == 1)
        {
    ...
    public static async void AdminChangePassword(string username, string password)
    {
        ...
        var data_change =
            Encoding.UTF8.GetBytes(mode.ToString() + '\r' + username + '\r' + hashedNewPass + '\r' + "" + '\n');
        await stream.WriteAsync(data_change);
